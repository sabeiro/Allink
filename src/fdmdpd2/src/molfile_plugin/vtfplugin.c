...
static int vtf_read_next_timestep(void *data, 
  ...
  if (d->coords == NULL) {
    /* initialize coords and velocities */
    d->coords = malloc(natoms*3*sizeof(float));
    d->velocities = malloc(natoms*3*sizeof(float));
  ...
  do {
	...
	if (sscanf(line, " %f %f %f %f %f %f", &x, &y, &z, &vx, &vy, &vz) == 6) {
		d->velocities[aid*3] = vx;
		d->velocities[aid*3+1] = vy;
		d->velocities[aid*3+2] = vz;
	} else {
		d->velocities[aid*3] = 0.;
		d->velocities[aid*3+1] = 0.;
		d->velocities[aid*3+2] = 0.;
	}
	aid++;
      } else {
	...
	d->coords[aid*3+1] = y;
	d->coords[aid*3+2] = z;
	if (sscanf(line, " %u %f %f %f %f %f %f", &aid, &x, &y, &z, &vx, &vy, &vz) == 7) {
		d->velocities[aid*3] = vx;
		d->velocities[aid*3+1] = vy;
		d->velocities[aid*3+2] = vz;
	} else {
  ...
  if (ts != NULL) {
    ...
    memcpy(ts->velocities, d->velocities, natoms*3*sizeof(float));
...
VMDPLUGIN_API int VMDPLUGIN_init() {
  ...
  vsfplugin.filename_extension = "vsf1";
...
#endif
#if vmdplugin_ABIVERSION > 10
    vtfplugin.read_timestep_metadata    = read_timestep_metadata;
#endif
  ...
  vcfplugin.filename_extension = "vcf1";
  ...
  vtfplugin.filename_extension = "vtf1";
